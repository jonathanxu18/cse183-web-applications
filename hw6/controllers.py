...
# URL Signer to sign urls
url_signer = URLSigner(session)

# Get first/last name from auth table given the email of current user
...
def add_properties(post, posts):
    email = post['email']
    post['author'] = get_name_from_email(email)
    posts.append(post)
...
# Querying all the posts from the post table
@action('posts', method="GET")
@action.uses(db, auth.user, session, url_signer.verify())
def get_posts():

    main_posts = db(db.post.is_reply == None).select().as_list()
    posts = []
    # Query first/last name from database
    for post in main_posts:
        replies = db(db.post.is_reply == post['id']).select().as_list()

        for reply in replies:
            add_properties(reply, posts)

        add_properties(post, posts)
...
# Uses current user email and specified post id to query specific element in post table
...
# Used for testing
# Deletes all posts in the post table
