...
# Overriding get_rating and set_rating from ThumbRater
...
def index():
    ...
    if db(db.images).count() == 0:
        setup()
    # Returns to the template the list of images.
    images = db(db.images).select().as_list()
    # I add a star rater to each image, so each image can be rated.
    for img in images:
        # __call__ used during image_rater()
        # Field 'rater' is set to ImageRater object
